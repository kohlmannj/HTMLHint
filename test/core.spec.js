...
describe("Core", () => {
    test("Set false to rule no effected should result in an error", () => {
    ...
    test("Not load default ruleset when use undefined ruleset should result in an error", () => {
        const code = '<P ATTR=\'1\' id="a">><div id="a"><img src="" a="1" a="2"/></div>';
        const messages = HTMLHint.verify(code);
        expect(messages).toHaveLength(9);
    });

    test("Not load default ruleset when use empty ruleset should result in an error", () => {
        const code = '<P ATTR=\'1\' id="a">><div id="a"><img src="" a="1" a="2"/></div>';
        const messages = HTMLHint.verify(code, {});
        expect(messages).toHaveLength(9);
    });

    test("Inline ruleset not worked should result in an error", () => {
    ...
    test("Show formated result should not result in an error", () => {
        const code =
